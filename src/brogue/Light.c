...
/*  The state of light on the current level, which can be saved and
	later restored  */
struct LIGHTING_STATE
{
	short lightmap[DCOLS + 1][DROWS + 1][3];
	short lightcells[DCOLS][DROWS][3];
};
...
/*  Save the lighting state for later restoration  */
LIGHTING_STATE *backUpLighting(void) {
	LIGHTING_STATE *lights;

	lights = malloc(sizeof(LIGHTING_STATE));
	memset(lights, 0, sizeof(LIGHTING_STATE));
	
	short i, j, k;
	for (i=0; i<DCOLS + 1; i++) {
		for (j=0; j<DROWS + 1; j++) {
			for (k=0; k<3; k++) {
				lights->lightmap[i][j][k] = lightmap[i][j].light[k];
			}
		}
	}
	...
	for (i=0; i<DCOLS; i++) {
		for (j=0; j<DROWS; j++) {
			for (k=0; k<3; k++) {
				lights->lightcells[i][j][k] = tmap[i][j].light[k];
			}
		}
	}
...
/*  Restore previously saved lighting state  */
void restoreLighting(LIGHTING_STATE *lights) {
	short i, j, k;
	for (i=0; i<DCOLS + 1; i++) {
		for (j=0; j<DROWS + 1; j++) {
			for (k=0; k<3; k++) {
				lightmap[i][j].light[k] = lights->lightmap[i][j][k];
			}
		}
	}
	...
	for (i=0; i<DCOLS; i++) {
		for (j=0; j<DROWS; j++) {
			for (k=0; k<3; k++) {
				tmap[i][j].light[k] = lights->lightcells[i][j][k];
			}
		}
	}
}

/*  Free the memory associated with a saved lighting state  */
void freeLightingState(LIGHTING_STATE *lights)
{
	free(lights);
}
