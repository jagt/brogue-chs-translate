...
short loadScoreBuffer() {
	...
	for (i=0; i<HIGH_SCORES_COUNT; i++) {
		...
		count = fscanf(scoresFile, "%li\t%li\t", &(scoreBuffer[i].score), &(scoreBuffer[i].dateNumber));
		...
		// load description
		desc = fgets(scoreBuffer[i].description, COLS, scoresFile);
		if (desc == NULL) {
			break;
		}
...
/*  Return a globally increasing count of milliseconds  */
int getTicks(void) {
	return currentConsole.getTicks();
}
...
void initializeBrogueSaveLocation() {
	...
	err = g_mkdir_with_parents(config_path, 0755);
	if (err)
	{
