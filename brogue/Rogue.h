...
typedef struct LIGHTING_STATE LIGHTING_STATE;
...
/*  The light in one of the four corners of a dungeon cell  */
typedef struct lightcorner {
	short light[3];
} lightcorner;
...
extern "C" {
	...
	void randomizeColor(
		int x, int y, color *baseColor, short randomizePercent);
    ...
    void restoreLighting(LIGHTING_STATE *lighting);
	void freeLightingState(LIGHTING_STATE *lighting);
	void updateLighting();
	...
	void drawButtonsInState(BROGUE_DRAW_CONTEXT *context, 
	...
	void drawButton(BROGUE_DRAW_CONTEXT *context, BROGUE_EFFECT *button_effect,
