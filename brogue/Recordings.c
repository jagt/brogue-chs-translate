...
void fillBufferFromFile() {
	...
	numread = fread((void *) inputRecordBuffer, 1, INPUT_RECORD_BUFFER, recordFile);
	assert(numread != 0);
	...
	for (;;) {
		...
		if (fscanf(annotationFile, "%lu\t", &(currentReadTurn)) != 1) {
			if (feof(annotationFile)) {
				...
				str = fgets(rogue.nextAnnotation, 5000, annotationFile);
				assert(str != NULL);
				continue;
			}
		}
...
void displayAnnotation() {
	if (rogue.playbackMode
		...
		if (!rogue.playbackFastForward) {
			...
			printTextBox(rogue.nextAnnotation, 0, &black, &white, NULL, 0);
...
void printPlaybackHelpScreen() {
	...
	rogue.playbackMode = false;
	helpDialog(helpPtr, PLAYBACK_HELP_LINE_COUNT);
	rogue.playbackMode = true;
}
...
void advanceToLocation(unsigned long destinationFrame) {
	...
	window = BrogueWindow_open(root, (COLS - 20) / 2 - 1, ROWS / 2 - 1, 22, 3);
	BrogueWindow_setColor(window, windowColor);
	context = BrogueDrawContext_open(window);
	effect = BrogueEffect_open(context, PROGRESS_BAR_EFFECT_NAME);
...
void copyFile(char *fromFilePath, char *toFilePath, unsigned long fromFileLength) {
	...
	for (n = 0; n < fromFileLength; n += m) {
		...
		numread = fread((void *) fileBuffer, 1, m, fromFile);
		assert(numread == m);
